...
{
    const char *file_path = MOUNT_POINT"/log.txt";
    ...
    if(fgets(line, sizeof(line), f) == NULL) {
        ESP_LOGE("sdread", "File empty or read error");
        fclose(f);
        vTaskDelete(NULL);
        return;
    }

    //Read one line per second
    while (fgets(line, sizeof(line), f) != NULL) {
        char *pos = strchr(line, '\n');
        if (pos)
            *pos = '\0';

        ESP_LOGI("sdread", "CSV: '%s'", line);
...
void app_main(void)
{
    ...
    xTaskCreate(sd_read_task, "sd_read_task", 4096, NULL, 5, NULL);
}
