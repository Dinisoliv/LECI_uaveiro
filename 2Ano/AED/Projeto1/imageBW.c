...
//   if (!success) {
//     conditionalCleanupCode();int second_pixel_value = 
//   }
//   return success;
//
...
Image ImageCreateChessboard(uint32 width, uint32 height, uint32 square_edge,
  ...
  //Saves the color as an int
  int first_pixel_value = (int)first_value;

  //Calculates the number of squares
  uint32 NsquaresX = width / square_edge;
  
  //Creates the squares
  for (uint32_t i = 0; i < width; i++) {
    newImage->row[i] = AllocateRLERowArray(3 * NsquaresX + 1);
    //Changes the color from line to line
    uint8 start_value = (i / square_edge) % 2 == 0 ? first_pixel_value: first_pixel_value ^ 1;
    uint8 current_value = start_value;
    //Alocate a rle row
    int* rle_row = AllocateRLERowArray(NsquaresX + 2);
    uint32 index = 0;

    //Creates the pattern, alternating the colors
    for (uint32 square = 0; square <= NsquaresX; square++) {
      rle_row[0] = current_value;
      rle_row[index++] = square_edge; //Length
      }

    //Last value of rle_row
    rle_row[index] = EOR;
...
Image ImageAND(const Image img1, const Image img2) { 
  ...
  for(uint32 i = 0; i < img1->height; i++){
    ...
    img->row[i] = CompressRow(img1->width, Img_res);
  }
