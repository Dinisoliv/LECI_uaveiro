...
// A: ... Yes, because all children are detroyed first, avoiding dangling pointers
...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {  
  ...
  int leftHeight = _treeGetHeightREC(root->left);
  int rightHeight = _treeGetHeightREC(root->right);
...
static int _BSTreeCheckNodesHeight(const struct _BSTreeNode* root) {
  ...
  if (_BSTreeCheckNodesHeight(root->left) == NULL) {
    return 1;
  }
  if (_BSTreeCheckNodesHeight(root->right) == NULL) {
    return 1;
  }
...
void BSTreeTestInvariants(const BSTree* header) {
  ...
  //       (_BSTreeCheckNodesHeight(header->root) == _treeGetHeightREC));
  // Is BST ?
