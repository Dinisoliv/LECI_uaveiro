...
int ListSearch(List* l, const void* p) {
  ...
  if (l->current != NULL && l->compare(l->current->item, p) <= 0) {
    node = l->current;
    pos = l->currentPos;
  } else {
    node = l->head;
  ...
  while (node != NULL) //end when bigger (+)
  {
...
void* ListRemoveCurrent(List* l) {
  ...
  if (l->currentPos == 0)
    ...
    for (int i = 0; i < l->currentPos; i++)
    {
      prev = prev->next;
    }
    struct _ListNode* toRemove = l->current;
    prev->next = toRemove->next;
    l->current = toRemove->next;
    l->current = toRemove->next;
